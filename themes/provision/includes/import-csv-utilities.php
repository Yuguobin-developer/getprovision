...
//                        $this->print_pre($acfFields);
                        echo '<select name="ic_utility_map['.$labels.']" id="cars">';
                            foreach ($acfFields as $field){
                                ...
                                if($field['type'] == 'group' || $field['type'] == 'relationship'){
                                    continue;
                                }
                                //count1
                                ...
                                if(array_key_exists('sub_fields',$field)){
                                    foreach($field['sub_fields'] as $sub_field){
                                        ...
                                        echo '<option value="'.$optionValue.'" '.selected($labels,$sub_field['label']).'>'.$sub_field['label'].'</option>';
                                    }
                                }else{
                                    echo '<option value="'.$field['name'].'"  '.selected($labels,$field['label']).'>'.$field['label'].'</option>';
                                }
...
//        unset($results[1]);
        ...
        $entries = $wpdb->get_results( $wpdb->prepare("SELECT DISTINCT zipcode, city, municipal, county, state, post_title, brand, sub_territory FROM $table_name WHERE post_title = '%s'", $post_title), ARRAY_A );
        ...
        if(is_array($entries) && !empty($entries)) {
            foreach($entries as $ekey => $eval) {
                ...
                $results = $wpdb->get_col( "SELECT zipcode FROM `$table_name_post_data` WHERE zipcode='".$eval['zipcode']."' AND city='".$eval['city']."' AND post_title='$post_title' AND post_id='$post_id'" );
                if(empty($results)) {
