...
function step_form_1_address(){
    ...
    if (isset($_POST['zipcode']) && !empty($_POST['zipcode'])) {
        ...
        if($utility){
            ...
            $stateCity["address"] = array_key_exists('address', $_POST) ? $_POST["address"] : null;
            ...
            $_SESSION['enrol-step-data']['address'] = $stateCity['address'];
            $_SESSION['enrol-step-data']['state'] = $stateCity['state'];
            $_SESSION['enrol-step-data']['city'] = $stateCity['city'];
    ...
    //        $return['url'] = '?k='.base64_encode($str);
        ...
        if($utility){
            ...
            $_SESSION['enrol-step-data']['address'] = array_key_exists('address', $_POST) ? $_POST["address"] : null;
	...
	if (isset($_POST['affiliate_id']) && !empty($_POST['affiliate_id'])) {
	    ...
	    //$str .= '&address=' . urlencode(ucwords(strtolower($_POST['address'])));
...
function populate_choices( $form ) {
    ...
    if (!empty($utility2) && empty($plan2)) {

        $plan1Utility = get_utility_by_plan($plan1);

	if ($utility2 == $plan1Utility->ID) {
    ...
    //echo "<pre>First plan = \n"; print_r([$utility1 => $plan1, $utility2 => $plan2,]); print_r(get_utility_by_plan($plan1)->ID); echo "\n2nd plan = \n"; print_r(get_utility_by_plan($plan2)); print_r($uty); print_r($form); echo "</pre>";
   ...
   foreach( $form['fields'] as &$field ) {
	...
	// Affiliate ID
        if ($field->id == 55 && (isset($_GET["a"]) && !empty($_GET["a"]))) {

            $field->defaultValue = $_GET["a"];
...
function add_data_attr_forimg( $field_content, $field ) {
    ...
    if ( $field->id == 54 ) {
        ...
        if(isset($accVals['account_number_message'])){
            $valMessage = 'Your SPID is 10 digits long.';//$accVals['account_number_message'];
            $field_content = str_replace( '<input', "<input data-mess='$valMessage'", $field_content );
        }
        return $field_content;
    }
    ...
    foreach( $form['fields'] as &$field ) {
        ...
        if ( $field->id == '5' ) {
            if(!empty( rgpost( 'input_5' ) ) ){
                ...
                if (!preg_match('/^\d+,? [0-9a-zA-Z ]+$/', $streetAdd1)) {
                    //preg_match('/^\\d+ [a-zA-Z ]+, \\d+ [a-zA-Z ]+, [a-zA-Z ]+$/', $input)
                    //#, Street name, Zip Code, City, Country is the address format.
                    $validation_result['is_valid'] = false;
                    $field->failed_validation = true;
                    $field->validation_message = 'Address should start with a number and then the street name.  Example: "123 Main St"';
                }
            }
        }
        ...
        if($field->id == '54'){
            if(empty(rgpost('input_54')) && empty(rgpost('input_11_1')) && (rgpost('input_42') == 'PGW' || rgpost('input_47') == 'PGW') ){
                $validation_result['is_valid'] = false;
                $field->failed_validation = true;
                $field->validation_message = 'This field is required.';
            }
...
/**
 * add on click evevent to GF_5 submit button for custom jQuery validation
 */
add_filter( 'gform_submit_button_5', 'add_submit_btn_onclick', 9, 2 );
function add_submit_btn_onclick( $button, $form ) {
    $dom = new DOMDocument();
    $dom->loadHTML( $button );
    $input = $dom->getElementsByTagName( 'input' )->item(0);
    $onclick = " return GravityFormValidation_5_parent(); ";
    $onclick .= $input->getAttribute( 'onclick' );
    $input->setAttribute( 'onclick', $onclick );
    
    $onkeypress = " return GravityFormValidation_5_parent(); ";
