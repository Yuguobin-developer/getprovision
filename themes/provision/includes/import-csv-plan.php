...
//                    $this->import_csv_routine();
    ...
    function map_csv_data_fields() {
        ...
        <form method="post" action="options.php">
            ...
            <p>
                ...
                <label for="insert_update">Update existing plans and their data partially or fully. This will not import new plans</label><br>
            </p>
            <p> 
    ...
    public function aj_import_csv_routine() {
        ...
        $insertNew = $deleteAll = $updateAll = false;
        if(isset($plan_dataMap['delete_old']) && ($plan_dataMap['delete_old'] == 'delete') ) { $dbStatus = $this->delete_plans_posts(); $deleteAll = true; $insertNew = true; }
        elseif(isset($plan_dataMap['delete_old']) && ($plan_dataMap['delete_old'] == 'insert') ) { $insertNew = true; }
        elseif(isset($plan_dataMap['delete_old']) && ($plan_dataMap['delete_old'] == 'update') ) { $updateAll = true; $insertNew = false; }
            ...
            while (($csv_data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                ...
                if ($row == 0) {
                    ...
                    */
                    ...
                    $post_id = $this->plan_exists($csv_data[$plan_dataMapFlipped['plan_id']]);
                    
                    if(empty($post_id) && $insertNew && !empty($csv_data[$plan_dataMapFlipped['post_title']])){
                        // Create new post.
                        $post_data = array(
                        ...
                        $post_id = wp_insert_post($post_data,true);
                            ...
                            echo '</tr>';
                    }elseif($updateAll && !empty ($post_id)){
                        echo '<tr><td>'.$post_id.'</td>';
                        foreach($csv_data as $key => $val){
                            //Plan Block
                            $fielId = $plan_dataMap[$key];
